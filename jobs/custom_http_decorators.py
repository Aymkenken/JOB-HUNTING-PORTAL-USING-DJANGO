"""
Custom HTTP decorators for views.
"""
...
# Create middleware-based decorator
...
def require_http_methods(request_method_list):
    ...
    """
    if not isinstance(request_method_list, (list, tuple)):
...
def etag(etag_func):
    """
    Decorator that only handles ETag processing.
    """
    return condition(etag_func=etag_func)

def last_modified(last_modified_func):
    """
    Decorator that only handles Last-Modified processing.
    """
...
def get_csrf_token(request):
    """
    View that returns the CSRF token for AJAX requests.
    """
